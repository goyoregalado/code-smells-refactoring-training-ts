...
export class BirthdayService {
  ...
  sendGreetings(
    ...
    this.send(
      this.greetingMessagesFor(this.employeesHavingBirthday(date)),
      smtpHost,
      smtpPort,
      sender
    );
  ...
  private greetingMessagesFor(
    employees: Array<Employee>
  ): Array<GreetingMessage> {
  ...
  private employeesHavingBirthday(today: OurDate): Array<Employee> {
    return this._employeesRepository
      .allEmployees()
      .filter((employee: Employee) => employee.isBirthday(today));
  }

  private send(
    messages: Array<GreetingMessage>,
    smtpHost: string,
    smtpPort: number,
    sender: string
  ): void {
